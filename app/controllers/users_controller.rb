class UsersController < ApplicationController
  before_action :check_admin_privileges!, only: [:new, :create]
  ...
  def new_password
    ...
    if user || (current_user and (current_user.id.to_s == params[:id] || current_user.role == "admin"))
  ...
  def change_password
    ...
    if @user.update_attributes(:password => params[:user][:password], :password_confirmation => params[:user][:password_confirmation])
      @user.email_activate
      flash[:success] = "Password is changed."
